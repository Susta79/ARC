...
Path_circular::Path_circular(Eigen::Vector3d P0, Eigen::Vector3d P1, Eigen::Vector3d P2, Eigen::Vector3d euler0, Eigen::Vector3d euler1, double N, double dt) : Path (P0, P1, euler0, euler1, N, dt){
...
Eigen::MatrixXd Path_circular::xyz_array(){
    int len_s = this->s.size();
    Eigen::Vector3d xyz;
    Eigen::MatrixXd xyz_array(len_s,3);
    for (size_t i = 0; i < len_s; i++){
        xyz = this->CC + this->CR*cos(this->s(i))*this->U + this->CR*sin(this->s(i))*this->V;
